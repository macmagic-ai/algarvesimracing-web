...
export default function PricingPage() {
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-12 sm:px-6">
      <h1 className="font-heading text-5xl font-bold text-zinc-900 sm:text-6xl">Pricing & Booking</h1>
        ...
        {packages.map((item) => (
          <Card key={item.name} className="shadow-none">
            <CardHeader>
              ...
              <CardDescription className="text-zinc-600">{item.details}</CardDescription>
            </CardHeader>
            <CardContent>
              <p className="font-heading text-5xl font-bold text-zinc-900">{item.price}</p>
              <p className="mt-2 text-sm font-medium text-zinc-600">Effective rate: {item.rate}</p>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      <section id="booking" className="mt-16 text-center">
        <h2 className="font-heading text-3xl font-semibold text-zinc-900">Ready to Book?</h2>
